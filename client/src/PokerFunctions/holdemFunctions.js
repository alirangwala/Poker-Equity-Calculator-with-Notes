...
const detectThreeOfAKind = function (board) {
  ...
  if (
    freqArray.includes(3) &&
    !freqArray.includes(2) &&
    !freqArray.includes(3, freqArray.indexOf(3) + 1)
  ) {
    return true;
  }
  return false;
...
const detectTwoPair = function (board) {
  ...
  if (
    freqArray.includes(2) &&
    freqArray.includes(2, freqArray.indexOf(2) + 1) &&
    !freqArray.includes(3)
  ) {
    return true;
  }
...
const handStrength = function (board) {
  ...
  if (detectStraightFlush(board)) {
    ...
    fullHouse.push(
      freqArray.lastIndexOf(2) > -1
        ? freqArray.lastIndexOf(2)
        : freqArray.lastIndexOf(3, freqArray.lastIndexOf(3) - 1)
    );
    return fullHouse;
    // full house if two 3 of a kinds
